...
export function FrontDeskDashboard({ onNavigate }) {
    const { todayCount, loading: attendanceLoading } = useAttendance();
    ...
    return (
        <div className="space-y-10">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
                <div>
                    <h2 className="text-accent text-[10px] font-black uppercase tracking-[0.4em] mb-1 leading-none">Front Desk Operations</h2>
                ...
                <div className="flex gap-4">
                    <button
                        ...
                        className="flex items-center gap-3 bg-primary text-surface px-6 py-4 rounded-3xl font-black uppercase tracking-widest text-xs hover:scale-105 active:scale-95 transition-all shadow-premium"
                    >
                    ...
                    <button
                        ...
                        className="flex items-center gap-3 bg-accent text-surface px-6 py-4 rounded-3xl font-black uppercase tracking-widest text-xs hover:scale-105 active:scale-95 transition-all shadow-gold"
                    >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                ...
                <Card subtitle="Instant Timeline" title="Recent Activity">
                    <div className="mt-6 space-y-4">
                        {isSyncing ? (
                            <div className="py-10 text-center text-text/10 font-black uppercase tracking-[0.5em] animate-pulse">
                                Syncing Timeline...
                            </div>
                        ) : recentActivity.length === 0 ? (
                            <div className="py-10 text-center text-text/10 font-black uppercase tracking-[0.2em]">
                            ...
                            <div key={item.id} className="group flex items-center justify-between p-4 bg-text/[0.02] border border-text/5 rounded-3xl hover:bg-surface transition-all">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <p className="text-text font-black text-sm uppercase">{item.name}</p>
                                        <p className="text-text/40 text-[10px] font-bold uppercase tracking-widest">{item.action}</p>
                                    </div>
                                </div>
                                <div className="text-right">
                                    <p className="text-text/60 font-black text-xs">{item.time}</p>
                        ...
                        <button className="w-full py-4 text-[10px] font-black uppercase tracking-[0.3em] text-text/20 hover:text-accent transition-colors">
                            View Detailed Logs
                        </button>
                    </div>
                </Card>
                    ...
                    <div className="mt-6 space-y-6">
                        <div className="relative group">
                            ...
                            <input
                                ...
                                className="w-full bg-text/[0.03] border border-text/5 rounded-[2rem] py-6 pl-16 pr-8 text-xs font-black uppercase tracking-widest focus:outline-none focus:border-accent/30 focus:bg-white transition-all shadow-inner"
                            />
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div className="p-6 bg-surface rounded-3xl border border-text/5 text-center group hover:border-accent/20 transition-all cursor-pointer">
                                <span className="block text-text/20 text-[9px] font-black uppercase tracking-[0.2em] mb-2">Expired</span>
                                <span className="text-2xl font-black ">14</span>
                            </div>
                            <div className="p-6 bg-surface rounded-3xl border border-text/5 text-center group hover:border-accent/20 transition-all cursor-pointer">
                                <span className="block text-text/20 text-[9px] font-black uppercase tracking-[0.2em] mb-2">Expiring Soon</span>
                        ...
                        <div className="bg-primary/5 p-6 rounded-[2.5rem] border border-primary/10 flex items-center justify-between">
                            <div>
                                <p className="text-primary text-[10px] font-black uppercase tracking-widest mb-1">Gate System Status</p>
                                <p className="text-text font-black text-lg  uppercase">Online & Operational</p>
                            </div>
            ...
            <div className="bg-accent rounded-[3rem] p-12 text-surface overflow-hidden relative shadow-gold">
                ...
                <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
                    <div className="text-center md:text-left">
                        <h3 className="text-4xl font-black  tracking-tighter uppercase mb-2">Shift Performance</h3>
                    ...
                    <div className="text-center md:text-right">
                        <p className="text-6xl font-black  tracking-tighter leading-none mb-2">98%</p>
                        <p className="text-surface/50 text-[9px] font-black uppercase tracking-widest">Efficiency Rating</p>
                    </div>
                </div>
            </div>
        </div>
