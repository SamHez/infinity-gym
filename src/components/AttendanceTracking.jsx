...
export function AttendanceTracking() {
    ...
    const { todayCount, checkIn } = useAttendance();
    const [localCheckedIn, setLocalCheckedIn] = useState([]);

    const handleCheckIn = async (id) => {
        const success = await checkIn(id);
        if (success) {
            setLocalCheckedIn([...localCheckedIn, id]);
        }
    };
    ...
    return (
        <div className="space-y-10 max-w-5xl mx-auto">
            ...
            <div className="flex justify-between items-center bg-card p-12 rounded-[3.5rem] border border-text/5 shadow-premium relative group">
                ...
                <div className="z-10">
                    <h2 className="text-accent text-[11px] font-black uppercase tracking-[0.5em] mb-2 leading-none ">Asset Presence</h2>
                    <p className="text-text text-5xl font-black  tracking-tighter leading-none uppercase">CHECK-IN</p>
                </div>
                <div className="flex items-center gap-10 z-10">
                    <div className="text-right">
                        <p className="text-accent font-black text-6xl tracking-tighter tabular-nums leading-none ">{todayCount}</p>
                        <p className="text-text/20 text-[10px] font-black uppercase tracking-[0.2em] mt-3">Daily Operations Pool</p>
                    </div>
            ...
            <Card className="p-3 border border-text/5 shadow-premium group">
                <div className="relative">
                    ...
                    <input
                        ...
                        className="w-full bg-surface/50 border-none rounded-[2.5rem] py-8 pl-24 pr-10 text-text font-black text-2xl focus:ring-4 focus:ring-accent/10 placeholder:text-text/10 transition-all  font-sans"
                        value={search}
            ...
            <div className="space-y-6">
                <div className="flex items-center gap-4 ml-4">
                    ...
                    <p className="text-text/20 text-[11px] font-black uppercase tracking-[0.5em] ">Tactical Suggestions</p>
                </div>

                {loading ? (
                    <div className="py-20 text-center text-text/10 font-black uppercase tracking-[1em] animate-pulse">
                        Scanning Bio-Matrix...
                    </div>
                ) : members.length === 0 ? (
                    <div className="py-20 text-center text-text/10 font-black uppercase tracking-[0.5em]">
                        No Personnel Available for Check-in
                    </div>
                ) : members.map(member => (
                    <Card key={member.id} className={cn(
                        ...
                        <div className="flex items-center gap-10">
                            <div className={cn(
                                ...
                                localCheckedIn.includes(member.id)
                                    ? "bg-success text-white shadow-premium scale-110"
                                    : "bg-surface text-text/10 group-hover:text-text/30"
                            )}>
                                {localCheckedIn.includes(member.id) ? <CheckCircle2 size={40} strokeWidth={2.5} /> : <UserCheck size={40} strokeWidth={2.5} />}
                            </div>
                            <div>
                                <div className="flex items-center gap-4 mb-1">
                                    <p className="text-text font-black text-3xl tracking-tighter  leading-none">{member.full_name}</p>
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    <span className="text-text/20 text-[10px] font-black uppercase tracking-widest tabular-nums">AUTH #{member.id}X924</span>
                                </div>
                            </div>
                        </div>

                        <button
                            disabled={localCheckedIn.includes(member.id) || member.status === 'Expired'}
                            onClick={() => handleCheckIn(member.id)}
                            className={cn(
                                "px-10 py-5 rounded-[2rem] font-black text-[11px] uppercase tracking-[0.2em] transition-all relative overflow-hidden",
                                localCheckedIn.includes(member.id)
                        ...
                        >
                            {localCheckedIn.includes(member.id) ? 'Identity Verified' : (member.status === 'Expired' ? 'Terminated Access' : 'Authorize Presence')}
                        </button>
                    </Card>
                ))}
            </div>
