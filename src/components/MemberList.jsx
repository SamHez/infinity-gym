    ...
    return (
        <div className="space-y-10">
            ...
            <div className="flex justify-between items-center bg-card p-10 rounded-[3rem] border border-text/5 shadow-premium relative overflow-hidden">
                ...
                <div>
                    <h2 className="text-accent text-[10px] font-black uppercase tracking-[0.4em] mb-1 leading-none ">Asset Database</h2>
                    <p className="text-text text-5xl font-black  tracking-tighter leading-none uppercase">DIRECTORY</p>
                </div>

                {/* Only Receptionist adds members based on prompt, but usually both can. Prompt says Receptionist add members. */}
                <button
                    onClick={onAddMember}
                    className="flex items-center gap-4 px-8 py-5 bg-primary text-white rounded-3xl shadow-premium hover:scale-105 active:scale-95 transition-all font-black text-xs uppercase tracking-widest z-10"
                >
            ...
            <Card className="p-2 border border-text/5 shadow-premium group">
                <div className="relative">
                    ...
                    <input
                        ...
                        className="w-full bg-surface/50 border-none rounded-[2rem] py-7 pl-20 pr-8 text-text font-black text-xl focus:ring-4 focus:ring-accent/10 placeholder:text-text/10 transition-all  font-sans"
                        value={search}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {loading ? (
                    <div className="col-span-2 py-20 text-center text-text/10 font-black uppercase tracking-[1em] animate-pulse">
                        Synchronizing Database...
                    </div>
                ) : members.length === 0 ? (
                    <div className="col-span-2 py-20 text-center text-text/10 font-black uppercase tracking-[0.5em]">
                        ...
                        <div className="flex items-center gap-8">
                            ...
                            <div className="space-y-1">
                                <div className="flex items-center gap-3">
                                    <p className="text-text font-black text-2xl tracking-tighter  leading-none">{member.full_name}</p>
                                    {member.status === 'Active' && <ShieldCheck size={18} className="text-success" />}
                                </div>
                                <div className="flex items-center gap-3">
                                    <span className="text-[10px] font-black text-text/20 uppercase tracking-[0.15em]">{member.category} ACCESS</span>
                                    <div className="w-1.5 h-1.5 bg-text/5 rounded-full" />
                                    <span className={cn(
                                        "text-[10px] font-black tracking-tight px-3 py-1 rounded-xl shadow-sm",
            ...
            <div className="pt-10 flex flex-col items-center gap-4">
                ...
                <p className="text-text/20 font-black text-xs uppercase tracking-[0.5em] ">
                    Viewing 1,248 Verified Personnel
                </p>
            </div>
        </div>
    );
